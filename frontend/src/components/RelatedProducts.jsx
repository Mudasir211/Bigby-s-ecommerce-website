       ...
       <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-5 gap-y-5'>
             {
               products.map((item,i)=>{
    if (i<=4){
        return <div  onClick={()=>{navigate(`/product/${item._id}`); window.scrollTo({ top: 0, behavior: 'smooth' });}} key={item._id} className='flex flex-col gap-2 text-xs cursor-pointer group'>
