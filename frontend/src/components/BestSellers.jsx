...
if (isfetching){return <p>Loading</p>}
  ...
  return (
     <div className='flex flex-col items-center gap-10 border-b-2 mb-14 pb-14'>
          <div className='flex flex-col items-center gap-4'><h2 className='flex items-center gap-3 text-2xl md:text-3xl outfit'><span className='opacity-70'>BEST </span>  SELLERS<span className='h-[2px] w-8 bg-black'></span></h2> <p className='opacity-70'>explore our bestseller collection!</p>
        </div>
       <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-5 gap-y-5'>
             {
               productData.map((item,i)=>{
    if (item.bestseller===true){
        return <div  onClick={()=>navigate(`/product/${item._id}`)} key={item._id} className='flex flex-col gap-2 text-xs break-all whitespace-normal cursor-pointer group'>
