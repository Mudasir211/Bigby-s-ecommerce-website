...
function Signin() {
  ...
  useEffect(() => {
    if (isError && error) {
      notify('error', error?.data?.message || 'Signup failed')
    } else if (isSuccess && data) {
      // Store token in localStorage (assumes backend returns token in data.token)
  ...
  return (
    <div>
      ...
      >
        ...
        <button className="px-8 py-2 mt-4 font-light text-white bg-black">Sign Up</button>
      </form>
      <GoogleSignIn />
      <ToastContainer />
    </div>
